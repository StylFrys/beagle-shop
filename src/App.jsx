...
const StarRating = ({ thc }) => {
  ...
  return <div style={{ display: 'flex', alignItems: 'center' }}>{[1,2,3,4,5].map(i => <GTAStar key={i} filled={i <= count} />)}</div>;
};
...
function App() {
  ...
  return (
    <div style={{ background: '#0a0a0a', backgroundImage: `url(${appBackground})`, backgroundAttachment: 'fixed', minHeight: '100vh', color: '#fff', paddingBottom: '100px' }}>
      ...
      {activeTab === 'cart' && <div style={{padding:'20px', color:'#fff', textAlign:'center'}}>Cart</div>}
      {activeTab === 'info' && <div style={{padding:'20px', color:'#fff', textAlign:'center'}}>Info</div>}
